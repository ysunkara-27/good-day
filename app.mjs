...
async function api(path,method='GET',body){let response;try{response=await fetch(`${API}/dog/${path}`,{method,headers:{...(token?{authorization:`Bearer ${token}`} :{}),...(window.siteAnalyticsId?{'x-analytics-id':window.siteAnalyticsId}:{}),...(body?{'content-type':'application/json'}:{})},body:body?JSON.stringify(body):undefined,signal:AbortSignal.timeout(15000)});}catch{throw new Error('Couldn’t connect. Your changes haven’t been saved. Please try again.');}let data;try{data=await response.json()}catch{throw new Error('The planner is unavailable. Please try again.');}if(!response.ok){if(response.status===401&&!['login','signup'].includes(path)){token='';localStorage.removeItem('good-day-token');showAuth();}if(response.status===409&&path.startsWith('day')){$('reload').hidden=false;}throw new Error(data.error||'Something went wrong. Please try again.');}return data;}
